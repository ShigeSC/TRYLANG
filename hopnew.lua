...
local SCRIPT_URL = "https://raw.githubusercontent.com/ShigeSC/TRYLANG/refs/heads/main/hopnew.lua"
...
local function sellSelectedBackpackPets()
    ...
    if backpack and Networking.NPCS and Networking.NPCS.SellPet then
        ...
        for petName, tools in pairs(sellCandidates) do
            for index = sellKeepCount + 1, #tools do
...
local function loadSettings()
    ...
    sellKeepCount = math.max(0, math.floor(tonumber(data.sellKeepCount) or 1))
...
local SellPetsPanel = CreatePanel(AutoBuyPage, "SellPetsPanel", UDim2.new(0, 264, 0, 12), UDim2.new(1, -276, 0, 200), "SELECTED PETS FOR SELL")
...
New("UICorner", { CornerRadius = UDim.new(0, 5) }, SellToggle)

New("TextLabel", {
    Text = "Keep copies (favorites/equipped are protected)",
    Font = Theme.FontBody,
    TextSize = 10,
    TextColor3 = Theme.TextDim,
    BackgroundTransparency = 1,
...
SellKeepBox = New("TextBox", {
    Name = "SellKeepBox", Text = "1", Font = Theme.Font, TextSize = 12,
    TextColor3 = Theme.InputText, BackgroundColor3 = Theme.InputBg, BorderSizePixel = 0,
    ClearTextOnFocus = false, Position = UDim2.new(1, -62, 0, 160), Size = UDim2.new(0, 52, 0, 22),
    TextXAlignment = Enum.TextXAlignment.Center,
}, SellPetsPanel)
New("UICorner", { CornerRadius = UDim.new(0, 5) }, SellKeepBox)
...
SellKeepBox.FocusLost:Connect(function()
    sellKeepCount = math.max(0, math.floor(tonumber(SellKeepBox.Text) or sellKeepCount))
    SellKeepBox.Text = tostring(sellKeepCount)
    saveSettings()
end)
    ...
    task.defer(function()
        ...
        for _, obj in ipairs(hitObjects) do
            if obj == PetDropdown or obj:IsDescendantOf(PetDropdown)
                or obj == SelectPetsButton or obj == SelectSellPetsButton or obj == RefreshPetsButton then
                clickedInside = true
                break
            end
        end
...
local ImportSettingsButton = New("TextButton", {
    ...
    BackgroundColor3 = Theme.Surface3,
    BorderSizePixel = 0,
    Position = UDim2.new(1/3, 4, 0, 148),
    Size = UDim2.new(1/3, -16, 0, 22),
}, SettingsPanel)
...
PresetsButton = New("TextButton", {
    ...
    Position = UDim2.new(2/3, -4, 0, 148), Size = UDim2.new(1/3, -8, 0, 22),
}, SettingsPanel)
...
updateSellUI = function()
    SellToggle.Text = autoSellEnabled and "SELL PET: ON" or "SELL PET: OFF"
...
local StatusLabel = CreateStatusCard("ProtectionCard", UDim2.new(0, 12, 0, 36), "PROTECTION", Theme.TextDim)
StatusLabel.Name = "StatusLabel"
StatusLabel.Text = "OFF"

local BoughtLabel = CreateStatusCard("SessionPetsCard", UDim2.new(0.5, 4, 0, 36), "SESSION PETS", Theme.Success)
local PointsLabel = CreateStatusCard("PointsCard", UDim2.new(0, 12, 0, 70), "POINTS", Color3.fromRGB(255, 208, 105))
local ShecklesLabel = CreateStatusCard("ShecklesCard", UDim2.new(0.5, 4, 0, 70), "SHECKLES", Color3.fromRGB(123, 222, 151))
