...
-- Searchable dropdown + Pets bought counter + Auto Rejoin + Config Save
-- Fixed: queue_on_teleport (Option B - GitHub raw) + proper resume
-- =====================================================
...
-- Option B: load from your GitHub raw
local SCRIPT_URL = "https://raw.githubusercontent.com/ShigeSC/TRYLANG/refs/heads/main/PET.lua"
...
-- Queue the script from GitHub so it re-executes after teleport
local function queueSelfForTeleport()
	...
	local source = "loadstring(game:HttpGet('" .. SCRIPT_URL .. "'))()"
	local queueOk = pcall(queueteleport, source)
	if not queueOk then
		return false, "The executor rejected the queued script."
	end
...
-- Safety net: also queue on any teleport if autoRejoin is enabled
LocalPlayer.OnTeleport:Connect(function()
	if autoRejoin then
		local queued, reason = queueSelfForTeleport()
		if not queued then
			warn("[Auto Buy Pet] Could not queue script: " .. tostring(reason))
		end
	end
end)
...
local function loadSettings()
	...
	petProtectEnabled = data.petProtectEnabled == true
end
...
task.defer(function()
	...
	local pillX = math.clamp(40 + titleWidth + 14, 200, 300)
...
RejoinToggle.Activated:Connect(function()
	...
	saveSettings()
	if autoRejoin then
		Notify("Auto Rejoin", "Will rejoin when no pets left", 2)
	else
...
-- =========================================================
-- PET PROTECT LOGIC + AUTO REJOIN
-- =========================================================
local function setPetProtectEnabled(enabled)
	petRunId = petRunId + 1
	local runId = petRunId
	...
	if petProtectEnabled then
		...
		petProtectThread = task.spawn(function()
			...
			local function findWildPets(names)
				...
				for _, pet in ipairs(spawns:GetChildren()) do
					if pet:IsA("Model") then
						local lowerPetName = string.lower(pet.Name)
			...
			local function securePet(pet)
				Notify("Target Locked", "Now protecting: " .. pet.Name, 2)
				noPetTimer = 0

				while petProtectEnabled and runId == petRunId do
					...
					local inSafeZone = isInsideSafeZone(petPos)
					...
					if intruder and intruder.Character then
						...
						if prompt and prompt.Enabled and not inSafeZone then
			...
			while petProtectEnabled and runId == petRunId do
				...
				if #matches == 0 then
					...
					if autoRejoin and noPetTimer >= NO_PET_TIMEOUT then
						Notify("Auto Rejoin", "No pets left — rejoining...", 3)
						saveSettings()

						-- Queue the script so it re-executes after teleport (Option B)
						local queued, reason = queueSelfForTeleport()
						if not queued then
							warn("[Auto Buy Pet] Could not queue script: " .. tostring(reason))
							Notify("Auto Rejoin", "Queue failed — enable AutoExecute as backup", 4)
						end
