...
local SCRIPT_URL = "https://raw.githubusercontent.com/ShigeSC/TRYLANG/refs/heads/main/nen.lua"
...
-- KRNL queue setup (call this when auto-rejoin is enabled)
...
RejoinToggle.Activated:Connect(function()
    ...
    -- Set up KRNL queue when enabled
    if autoRejoin then
        enableKRNLQueue()
        Notify("Auto Rejoin", "Will rejoin when no pets left (KRNL: " .. tostring(isKRNL) .. ")", 2)
    else
...
Notify("Loaded", "Settings restored • KRNL: " .. tostring(isKRNL), 3)
