...
local SCRIPT_URL = "https://raw.githubusercontent.com/ShigeSC/TRYLANG/refs/heads/main/hopmen.lua"
...
runPoints = 0
recentPointEvents = {}
hourlyPointsCurrent = 0
hourlyPointsLast = 0
hourlyActiveSeconds = 0
hourlyLastTick = 0
hourlyHasCompleted = false
hourlyLastSavedAt = 0
...
local function storeCurrentPlayerStats()
    playerStats[currentPlayerKey] = {
        ...
        runPoints = runPoints,
        hourlyPointsCurrent = hourlyPointsCurrent,
        hourlyPointsLast = hourlyPointsLast,
        hourlyActiveSeconds = hourlyActiveSeconds,
        hourlyHasCompleted = hourlyHasCompleted,
        history = petHistory,
    }
end
...
local function getRecentPointsPerMinute()
    local now = os.clock()
    local total = 0
    local kept = {}
    for _, event in ipairs(recentPointEvents) do
        if now - event.at <= 60 then
            total = total + event.points
            table.insert(kept, event)
        end
    end
    recentPointEvents = kept
    return total
end

local function advanceHourlyPointsWindow()
    if not petProtectEnabled then
        return
    end

    local now = os.time()
    if hourlyLastTick <= 0 then
        hourlyLastTick = now
        return
    end

    -- Preserve normal rejoin time, but do not count long offline periods.
    local elapsed = math.clamp(now - hourlyLastTick, 0, 30)
    hourlyLastTick = now
    hourlyActiveSeconds = hourlyActiveSeconds + elapsed

    while hourlyActiveSeconds >= 3600 do
        hourlyPointsLast = hourlyPointsCurrent
        hourlyPointsCurrent = 0
        hourlyActiveSeconds = hourlyActiveSeconds - 3600
        hourlyHasCompleted = true
    end
end
...
local function loadSettings()
    ...
    if data.dailyDate == os.date("%Y-%m-%d") then
        dailyDate = data.dailyDate
        dailyPetsBought = tonumber(data.dailyPetsBought) or 0
        dailyPoints = tonumber(data.dailyPoints) or 0
    else
        dailyDate = os.date("%Y-%m-%d")
        dailyPetsBought = 0
        dailyPoints = 0
    ...
    if type(savedStats) == "table" then
        ...
        runPoints = math.max(0, tonumber(savedStats.runPoints) or 0)
        hourlyPointsCurrent = math.max(0, tonumber(savedStats.hourlyPointsCurrent) or 0)
        hourlyPointsLast = math.max(0, tonumber(savedStats.hourlyPointsLast) or 0)
        hourlyActiveSeconds = math.max(0, tonumber(savedStats.hourlyActiveSeconds) or 0)
        hourlyHasCompleted = savedStats.hourlyHasCompleted == true
...
local SettingsPage = New("ScrollingFrame", {
    ...
    Position = UDim2.new(0, 0, 0, 42),
    Size = UDim2.new(1, 0, 1, -48),
    CanvasSize = UDim2.new(0, 0, 0, 618),
...
-- =========================================================
local PetsPanel = CreatePanel(SettingsPage, "PetsPanel", UDim2.new(0, 12, 0, 12), UDim2.new(0, 240, 0, 126), "BUY TARGET PETS")
...
-- =========================================================
SettingsPanel = CreatePanel(SettingsPage, "SettingsPanel", UDim2.new(0, 12, 0, 224), UDim2.new(1, -24, 0, 170), "BUY & MOVEMENT")
...
local ImportSettingsButton = New("TextButton", {
    ...
    BackgroundColor3 = Theme.Surface3,
    BorderSizePixel = 0,
    Position = UDim2.new(1/3, 4, 0, 146),
    Size = UDim2.new(1/3, -16, 0, 22),
}, SettingsPanel)
...
PresetsButton = New("TextButton", {
    ...
    BackgroundColor3 = Theme.Surface3, BorderSizePixel = 0,
    Position = UDim2.new(2/3, -4, 0, 146), Size = UDim2.new(1/3, -8, 0, 22),
}, SettingsPanel)
...
-- =========================================================
JobIdPanel = CreatePanel(SettingsPage, "JobIdPanel", UDim2.new(0, 12, 0, 406), UDim2.new(1, -24, 0, 200), "SERVER HOP ROTATION")
...
local function CreateStatusCard(name, position, labelText, valueColor)
    local isLarge = name == "ElapsedCard" or name == "PointsCard"
...
local PointsLabel = CreateStatusCard("PointsCard", UDim2.new(0, 12, 0, 36), "TOTAL POINTS", Color3.fromRGB(255, 208, 105))
PointsLabel.Size = UDim2.new(1, -112, 0, 25)
local PointsRateLabel = New("TextLabel", {
    Name = "PointsRateLabel",
    Text = "+0 | 0:00",
    Font = Theme.Font,
    TextSize = 12,
    TextColor3 = Theme.Success,
...
local LastHourPointsLabel = New("TextLabel", {
    Name = "LastHourPointsLabel",
    Text = "LAST --/h",
    Font = Theme.Font,
    TextSize = 8,
    TextColor3 = Theme.Muted,
...
ElapsedLabel = CreateStatusCard("ElapsedCard", UDim2.new(0.5, 4, 0, 36), "ELAPSED", Theme.White)
local BoughtLabel = CreateStatusCard("SessionPetsCard", UDim2.new(0, 12, 0, 92), "SESSION PETS", Theme.Success)
local ShecklesLabel = CreateStatusCard("ShecklesCard", UDim2.new(0.5, 4, 0, 92), "SHECKLES", Color3.fromRGB(123, 222, 151))
SpentLabel = CreateStatusCard("SpentCard", UDim2.new(0, 12, 0, 126), "SPENT", Color3.fromRGB(255, 166, 112))
ServerHopsLabel = CreateStatusCard("ServerHopsCard", UDim2.new(0.5, 4, 0, 126), "SERVER HOPS", Theme.Text)
...
updateHistoryUI = function()
    ...
    HistoryDailyLabel.Text = "Today: " .. dailyPetsBought .. " pets • " .. dailyPoints .. " points"
...
local function updateStatusUI()
    ...
    advanceHourlyPointsWindow()
    PointsRateLabel.Text = "+" .. formatWebhookNumber(runPoints) .. " | " .. formatWebhookElapsed()
    PointsRateLabel.Position = UDim2.new(0, 16 + PointsLabel.TextBounds.X, 0, 34)
...
function beginServerHop()
    ...
    task.spawn(function()
        ...
        if targetJobId then
            ...
            if not server then
                serverHopInProgress = false
                Notify("Server Hop", "No 1-6 player server was found.", 3)
                return
            end
...
local function setPetProtectEnabled(enabled)
    ...
    if petProtectEnabled and hourlyLastTick <= 0 then
        hourlyLastTick = os.time()
    elseif not petProtectEnabled then
        hourlyLastTick = 0
    end
    rebuildTargetList()
    updateStatusUI()
    ...
    if petProtectEnabled then
        ...
        petProtectThread = task.spawn(function()
            ...
            local function forceRun()
                ...
                humanoid.WalkSpeed = fastApproach and math.max(petWalkSpeed, 70) or petWalkSpeed
            ...
            local function findWildPets(names)
                ...
                table.sort(matches, function(first, second)
                    local _, firstPoints = getPetRarityAndPoints(first)
                    local _, secondPoints = getPetRarityAndPoints(second)
            ...
            local function recordSecuredPet(pet, petName, purchasePrice, purchaseRequested)
                local rarity, points = getPetRarityAndPoints(pet)
                petsBought = petsBought + 1
                totalPoints = totalPoints + points
                runPoints = runPoints + points
                table.insert(recentPointEvents, { at = os.clock(), points = points })
                advanceHourlyPointsWindow()
                hourlyPointsCurrent = hourlyPointsCurrent + points
                ...
                addActivity("Secured " .. petName .. " (" .. rarity .. ")")
            ...
            local function securePet(pet)
                ...
                while petProtectEnabled do
                    ...
                    if not pet.Parent then
                        ...
                        recordSecuredPet(pet, expectedPetName, purchasePrice, purchaseRequested)
                        aimAtNextWildPet()
                        break
                    elseif false then
                        fastApproach = false
                        runTarget = nil
                        local acquired = waitForPetInBackpack(pet, expectedPetName, ownedBefore, petRef)
                        if not acquired then
                            addActivity("Lost " .. expectedPetName .. " to another player")
                            Notify("Pet Lost", expectedPetName .. " was taken. Moving to the next target.", 2)
                            aimAtNextWildPet()
                            break
                        end
                        if acquired then
                        ...
                        advanceHourlyPointsWindow()
                        hourlyPointsCurrent = hourlyPointsCurrent + points
                        if purchaseRequested and purchasePrice > 0 then
                            totalSpent = totalSpent + purchasePrice
                        end
                        if dailyDate ~= os.date("%Y-%m-%d") then
                            dailyDate = os.date("%Y-%m-%d")
                    ...
                    if intruder and intruder.Character then
                        ...
                        if otherHRP then
                            -- Keep moving toward the WildPet after punching. This
                            -- prevents the movement loop from chasing players away
                            -- from the purchase prompt, while preserving the radius.
                            runTarget = petPos
...
task.spawn(function()
    while ScreenGui.Parent do
        ...
        if PointsRateLabel then
            advanceHourlyPointsWindow()
            PointsRateLabel.Text = "+" .. formatWebhookNumber(runPoints) .. " | " .. formatWebhookElapsed()
            PointsRateLabel.Position = UDim2.new(0, 16 + PointsLabel.TextBounds.X, 0, 34)
        end
        if petProtectEnabled and os.time() - hourlyLastSavedAt >= 15 then
            hourlyLastSavedAt = os.time()
            saveSettings()
        end
